...
def _images_show_damage(image_results: list[ImageAnalysisResult]) -> bool:
...
def decide(
    ...
    3. Object mismatch            → contradicted
    4. Low confidence             → not_enough_information
    5. Part visible, no damage    → contradicted
    6. Damage matches claim       → supported
    7. Damage does not match      → contradicted
    8. Default                    → not_enough_information
    """
    all_flags = _merge_risk_flags(image_results, risk_result)
    ...
    if not overall_valid or best is None:
        return DecisionResult(
            ...
            claim_status_justification="No valid images were provided. Cannot evaluate the claim.",
    ...
    if not _object_matches(image_results, claim_object):
        ...
        return DecisionResult(
            ...
            claim_status_justification=(
                ...
                f"Detected: {best.detected_object}. Claim cannot be verified."
            ),
            ...
            severity=best.severity,
            valid_image=True,
            risk_flags=["wrong_object"] + [f for f in all_flags if f != "wrong_object"],
        )
    ...
    if best.confidence.overall < CONFIDENCE_THRESHOLD:
        return DecisionResult(
            ...
            claim_status_justification=(
                ...
                "to make a reliable decision. Manual review recommended."
            ),
            supporting_image_ids=[],
            issue_type=best.issue_type,
            object_part=best.object_part,
            severity=best.severity,
            valid_image=True,
            risk_flags=all_flags + ["manual_review_required"],
        )

    # Rule 5: part visible, no damage → contradicted
    if best.issue_type == "none" and best.object_part != "unknown":
        ids = _select_supporting_images(image_results, "contradicted", best.issue_type)
        return DecisionResult(
            claim_status="contradicted",
            claim_status_justification=(
    ...
    # Rule 6: damage matches claim → supported
    if _images_show_damage(image_results) and best.issue_type == claim_result.claimed_issue:
        ids = _select_supporting_images(image_results, "supported", best.issue_type)
        return DecisionResult(
            claim_status="supported",
            claim_status_justification=(
                f"{best.image_id} shows visible {best.issue_type} on the "
                f"{best.object_part}, consistent with the reported claim."
            ),
    ...
    # Rule 7: damage visible but does not match claim → contradicted
    if _images_show_damage(image_results):
        ids = _select_supporting_images(image_results, "contradicted", best.issue_type)
        return DecisionResult(
            claim_status="contradicted",
            claim_status_justification=(
                f"{best.image_id} shows {best.issue_type} on the {best.object_part}, "
                f"but the claim states {claim_result.claimed_issue} on "
    ...
    return DecisionResult(
        claim_status="not_enough_information",
        claim_status_justification="Could not determine claim status from the available images.",
        supporting_image_ids=[],
        issue_type=best.issue_type,
        object_part=best.object_part,
        severity=best.severity,
